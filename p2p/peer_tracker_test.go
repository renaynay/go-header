...
func TestPeerTracker_GC(t *testing.T) {
	...
	p, err := newPeerTracker(h[0], connGater, pidstore, false)
	require.NoError(t, err)
...
func TestPeerTracker_BlockPeer(t *testing.T) {
	...
	p, err := newPeerTracker(h[0], connGater, nil, false)
	require.NoError(t, err)
...
func TestPeerTracker_Bootstrap(t *testing.T) {
	...
	tracker, err := newPeerTracker(mn.Hosts()[0], connGater, pidstore, false)
	require.NoError(t, err)
